...
export function DataMedia({
  ...
  documentGridCols = "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3",
}: DataMediaProps) {
  ...
  const MediaContent = () => (
    ...
    >
      <TabsList className="grid w-full grid-cols-3">
        ...
        <TabsTrigger
          value="documents"
          disabled={mediaFiles.application.length === 0}
        >
          <FileText className="h-4 w-4 mr-2" />
          Tài liệu ({mediaFiles.application.length})
        </TabsTrigger>
      </TabsList>
          ...
          {mediaFiles.video.map((videoUrl, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/40 group-hover:bg-black/60 transition-colors duration-300 flex items-center justify-center">
                <div className="bg-white rounded-full p-6 group-hover:scale-110 group-hover:bg-primary transition-all duration-300 shadow-xl">
                  <VideoIcon className="h-10 w-10 text-primary group-hover:text-white transition-colors duration-300" />
                </div>
              </div>
              <div className="absolute bottom-4 left-4 right-4 bg-black/80 backdrop-blur-sm text-white text-sm px-4 py-2 rounded-lg font-medium">
                Video {index + 1}
              </div>
            </div>
          ))}
      ...
      <TabsContent value="documents" className="mt-4">
        <div className={`grid ${documentGridCols} gap-4`}>
          {mediaFiles.application.map((docUrl, index) => {
            ...
            return (
              <div
                key={index}
                className="relative group cursor-pointer overflow-hidden rounded-xl border-2 border-border hover:border-primary hover:shadow-lg transition-all duration-300 p-6 bg-card"
              >
  ...
  return (
    <>
      ...
      {selectedImageIndex !== null && mediaFiles && (
        ...
        >
          <DialogContent
            className="max-h-[98vh] p-0 border-0 bg-black/95"
          ...
          >
            <div className="relative h-[80vh] flex items-center justify-center overflow-hidden">
              ...
              {/* Image Counter */}
              <div className="absolute top-6 left-6 z-20 bg-white/90 backdrop-blur-sm rounded-full px-4 py-2 shadow-xl">
                <span className="text-sm font-medium">
                  {selectedImageIndex + 1} / {mediaFiles.image.length}
                </span>
              </div>

              {/* Zoom Controls + Navigation */}
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 bg-white/90 backdrop-blur-sm rounded-full px-4 py-3 shadow-xl">
                <Button
                  variant="ghost"
                  size="icon"
      ...
      {selectedVideoIndex !== null && mediaFiles && (
        <Dialog open={true} onOpenChange={() => setSelectedVideoIndex(null)}>
          <DialogContent
            className="max-w-[95vw] max-h-[95vh] p-6 bg-black/95"
            showCloseButton={false}
          >
            <DialogHeader className="pb-4">
              <DialogTitle className="text-xl font-semibold">
                <div className="text-white">
                  {" "}
                  Xem video ({selectedVideoIndex + 1} /{" "}
            ...
            {mediaFiles.video.length > 1 && (
              <div className="flex items-center gap-2 bg-white/90 backdrop-blur-sm rounded-full px-4 py-3 shadow-xl justify-center">
