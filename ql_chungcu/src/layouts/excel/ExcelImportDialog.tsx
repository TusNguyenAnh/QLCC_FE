...
export function ExcelImportDialog<T>({
  ...
  // Cleanup khi unmount để đảm bảo backdrop được remove
  useEffect(() => {
    return () => {
      // Force cleanup backdrop khi component unmount
      const backdrop = document.querySelector("[data-radix-dialog-overlay]");
      if (backdrop) {
        backdrop.remove();
      }
      // Reset pointer-events trên body
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange} modal={true}>
      <DialogContent
        className="sm:max-w-[95vw] sm:max-h-[90vh] overflow-y-auto w-fit"
