...
export default function AptForm({open, setOpen, loading, action, formData, items, onSubmit}: ComponentProps) {
    const {
        register,
        handleSubmit,
        control,
        ...
        <Sheet open={open} onOpenChange={setOpen}>
            <SheetContent className="sm:max-w-[425px] flex flex-col">
                ...
                <form className="flex flex-col flex-1 relative"
                    ...
                    <SheetHeader>
                        <SheetTitle>
                            {action === "CREATE" ? "Thêm mới căn hộ" : "Cập nhật thông tin căn hộ"}
                        </SheetTitle>
                        <SheetDescription>
                            {action === "CREATE"
                    ...
                    <div className="grid auto-rows-min px-4 h-[75vh] overflow-y-auto">
                        <div className="grid gap-4">
                            ...
                            {/* Diện tích căn hộ */}
                            <div className="grid gap-3">
                                <Label htmlFor="apt_area">Diện tích (m<sup
                                    style={{margin: '0 -6px', fontSize: '10px'}}>2</sup>)</Label>
                                <Input id="apt_area" {...register("apt_area", {valueAsNumber: true})} type="number"
                                       min="10" max="1000"
                                       step="0.01"/>
                                {errors.apt_area &&
                                    <p className="text-sm text-red-500">{errors.apt_area.message}</p>}
                            </div>
