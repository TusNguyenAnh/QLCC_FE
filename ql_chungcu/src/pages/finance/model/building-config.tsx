...
export default function BuildingConfig({
    ...
    useEffect(() => {
        const total = buildings.reduce((sum, b) => sum + (b.financial_ratio || 0), 0);
    ...
    const validate = () => {
        ...
        buildings.forEach((building) => {
            if (!building.id.trim()) {
                newErrors[`name_${building.id}`] = "Tên tòa nhà không được để trống";
            }
        });
            ...
            <div className="w-full max-w-5xl">
                ...
                <Card className="shadow-2xl border-2 border-gray-200">
                    <CardContent className="p-6">
                                    ...
                                    <CardContent className="space-y-4 pt-4 flex justify-between gap-2">
                                        ...
                                        <div className="space-y-2 flex-1">
                                            ...
                                            <Select
                                                value={building.id}
                                                onValueChange={(value) =>
                                                    updateBuilding(building.id, "id", value)
                                                }
                                            >
                                                <SelectTrigger
                                                    className={`w-full text-base ${
                                                        errors[`name_${building.id}`]
                                        ...
                                        <div className="space-y-2 flex-1">
                                            <Label
                                                htmlFor={`percentage-${building.id}`}
                                                className="text-sm font-semibold flex items-center gap-2"
                                            >
                                                <Building2 className="h-4 w-4 text-indigo-600"/>
                                                Tỉ lệ phân bổ (%){" "}
                        ...
                        {/* Summary Section */}
                        <Card
                            className="mt-6 bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 border-2 shadow-lg">
                            <CardContent className="p-6">
                                ...
                                <div className="flex justify-center">
                                    <div
                                        className="p-6 bg-white rounded-xl shadow-md border-2 border-indigo-300 min-w-[280px]">
                                        ...
                                        {Math.abs(totalPercentage - 100) < 0.01 ? (
                                            <div
                                                className="text-sm text-green-600 font-medium text-center flex items-center justify-center gap-1">
